...
class NOAA(UTIL):
    ...
    def aw_forecast(self, lat, long, hourly=True):
        """Get observation data from a Aeris Weather.
        ...
        uri =  "https://api.aerisapi.com/forecasts/{lat},{long}?limit=14&client_id={ID}&client_secret={SECRET}".format(
                ID=self.AW_CLIENT_ID, SECRET=self.AW_SECRET_KEY, lat=lat, long=long)

        if hourly:
            uri = "https://api.aerisapi.com/forecasts/{lat},{long}?filter=1hr&limit=999&client_id={ID}&client_secret={SECRET}".format(
                ID=self.AW_CLIENT_ID, SECRET=self.AW_SECRET_KEY, lat=lat, long=long)
